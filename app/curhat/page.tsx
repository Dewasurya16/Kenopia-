        ...
        <main className="flex-1 flex flex-col min-w-0 h-full relative" style={{ zIndex: 10 }}>
          ...
          {!zenMode && (
            <header className="flex items-center justify-between px-4 md:px-6 py-3 flex-shrink-0"
              ...
              {/* Mobile controls */}
              <div className="flex md:hidden items-center gap-1.5 flex-shrink-0 ml-2">
                {streakCount >= 2 && <StreakBadge count={streakCount} />}
                <button onClick={toggleTheme} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center text-base transition-all hover:scale-110"
                  style={{ background: 'var(--surface-2)', border: '1px solid var(--border-2)', color: 'var(--text-faint)' }}>
                  {dark ? '☀️' : '🌙'}
                </button>
                <div className="relative" ref={mobileMenuRef}>
                  <button onClick={() => setShowMobileMenu(v => !v)}
                    className="h-8 sm:h-9 px-3 rounded-xl flex items-center gap-1.5 text-xs font-bold transition-all hover:scale-105"
                    style={{ background: showMobileMenu ? '#3b82f6' : 'var(--surface-2)', border: `1px solid ${showMobileMenu ? 'transparent' : 'var(--border-2)'}`, color: showMobileMenu ? 'white' : 'var(--text-faint)' }}>
                    <span className="text-sm">{persona === 'sahabat' ? '👋' : persona === 'psikolog' ? '🩺' : '🧘'}</span>
                    <span className="text-xs opacity-60 transition-transform inline-block" style={{ transform: showMobileMenu ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</span>
                  </button>
                </div>
              </div>
            </header>
