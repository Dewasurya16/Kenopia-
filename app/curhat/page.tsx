...
import { 
  FadingCanvasOverlay, HopeBalloonOverlay, TimeCapsuleOverlay, 
  FutureSelfOverlay, GroundingOverlay, BubbleWrapOverlay, 
  BreathingOverlay, BurnBebanOverlay, TimeCapsule, triggerVibrate 
    ...
    <motion.div initial={{ opacity: 0, y: 15, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="flex flex-col gap-2 mb-6">
      <div className="flex justify-end items-end gap-2 sm:gap-3">
        <div className="max-w-[85%] sm:max-w-[75%] flex flex-col items-end">
          <div className="px-5 sm:px-6 py-3.5 sm:py-4 text-[15px] leading-relaxed bubble-user font-medium opacity-60">
            {text}
          </div>
          <p className="text-right text-[11px] mt-1.5 font-bold flex items-center justify-end gap-1" style={{ color: 'var(--text-faint)' }}>
            <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ duration: 1.5, repeat: Infinity }}>⏳</motion.span> Mengirim...
          </p>
        </div>
        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-[11px] font-black text-white shadow-md flex-none opacity-60"
    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} layout className="flex flex-col gap-2 mb-6 msg-group">
      ...
      <div className="flex justify-end items-end gap-2 sm:gap-3">
        <div className="max-w-[85%] sm:max-w-[80%] flex flex-col items-end">
          ...
          <div className="px-5 sm:px-6 py-3.5 sm:py-4 text-[15px] leading-relaxed bubble-user font-medium">
            {msg.userMessage}
          </div>
          <p className="text-right text-[11px] mt-1.5 font-bold msg-time pr-1" style={{ color: 'var(--text-faint)' }}>{timeStr}</p>
        </div>
        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-xs font-black text-white shadow-md flex-none"
          style={{ background: 'linear-gradient(135deg,#1d4ed8,#3b82f6)' }}>
...
export default function CurhatPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Ekstrak Long Term Memory (Buku Catatan AI)
    const previousSessions = sessions.filter(s => s.id !== activeSessionId).slice(-3);
    const longTermMemory = previousSessions.length > 0 
       ? previousSessions.map(s => `- Sesi tgl ${new Date(s.createdAt).toLocaleDateString('id-ID')}: Topik awal = "${s.messages[0]?.userMessage}"`).join('\\n')
       : "";

    // Bawa pesan pertama (akar masalah) dan 15 pesan terakhir agar ingatan AI lebih kuat dan panjang
    const firstMsg = activeMessages.length > 0 ? [activeMessages[0]] : [];
    const recentMsgs = activeMessages.slice(-15);
    const combinedMsgs = Array.from(new Set([...firstMsg, ...recentMsgs]));
    const recentContext = combinedMsgs.map(h => ({ user: h.userMessage, ai: h.aiResponse }))
    ...
    try {
      ...
      if (!res.ok) throw new Error('Terjadi kesalahan pada server. Coba lagi.')
      
      const emotion = (res.headers.get('X-Emotion') || 'sedih') as EmotionKey;
      ...
      // Init message dengan empty string untuk mulai streaming efek ngetik
      const initMsg: ChatMessage = {
        id: newMsgId, userMessage: text, emotion, aiResponse: "", suggestions: [], timestamp: new Date().toISOString()
      }
      
      setSessions(prev => {
        const exist = prev.find(s => s.id === newSessionId)
        if (exist) return prev.map(s => s.id === newSessionId ? { ...s, messages: [...s.messages, initMsg] } : s)
        const words = text.split(' ')
        return [...prev, { id: newSessionId, title: words.length > 5 ? words.slice(0, 5).join(' ') + '...' : text, createdAt: new Date().toISOString(), messages: [initMsg] }]
      })
            ...
            if (line.startsWith('data: ') && line.trim() !== 'data: [DONE]') {
              try {
                ...
                setSessions(prev => prev.map(s => s.id === newSessionId ? {
                  ...s, messages: s.messages.map(m => m.id === newMsgId ? { ...m, aiResponse: currentAiResponse } : m)
                } : s))
              } catch (e) {}
            }
          }
      ...
      // Ekstrak <saran> dari respons akhir
      const saranMatch = currentAiResponse.match(/<saran>(.*?)<\/saran>/);
      if (saranMatch) {
         currentSuggestions = saranMatch[1].split('|').map(s => s.trim());
         currentAiResponse = currentAiResponse.replace(/<saran>.*?<\/saran>/g, '').trim();
         
         setSessions(prev => prev.map(s => s.id === newSessionId ? {
            ...s, messages: s.messages.map(m => m.id === newMsgId ? { ...m, aiResponse: currentAiResponse, suggestions: currentSuggestions } : m)
         } : s))
      }
      
      // Simpan ke local storage di akhir stream
      setSessions(prev => {
        localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(prev))
        setStreakCount(computeStreak(prev))
        return prev
      })
      
      // Eksekusi Voice AI jika aktif
    ...
    <>
      ...
      <div className="kp-app flex h-[100dvh] w-full overflow-hidden relative transition-colors duration-1000" style={{ background: mainBgColor } as React.CSSProperties}>
        ...
        <main className="flex-1 flex flex-col min-w-0 h-full relative" style={{ zIndex: 10 }}>
          ...
          {!zenMode && (
            <header className="flex items-center justify-between px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 flex-shrink-0 safe-top"
              ...
              <div className="hidden md:flex items-center gap-2 flex-shrink-0 ml-3">
                ...
                <motion.button whileHover={{ scale: 1.05 }} onClick={() => setIsVoiceMode(!isVoiceMode)} className="w-9 h-9 rounded-full flex items-center justify-center text-base transition-all" style={{ background: isVoiceMode ? 'rgba(59,130,246,0.1)' : 'var(--surface-2)', border: `1px solid ${isVoiceMode ? '#3b82f6' : 'var(--border-2)'}` }} title={isVoiceMode ? 'Suara AI Aktif' : 'Suara AI Nonaktif'}>{isVoiceMode ? '🔊' : '🔇'}</motion.button>
              ...
              <div className="flex md:hidden items-center gap-2 flex-shrink-0 ml-auto">
                ...
                <div className="relative">
                  <button onClick={() => setShowMobileMenu(v => !v)} className="w-10 h-10 rounded-2xl flex items-center justify-center text-xl transition-all active:scale-95 shadow-sm"
                    style={{ background: 'var(--surface-2)', border: '1px solid var(--border-2)', color: 'var(--text)' }}>⋮</button>
                  
                  <AnimatePresence>
                    {showMobileMenu && (
                       <>
                         <div className="fixed inset-0" style={{ zIndex: 90 }} onClick={() => setShowMobileMenu(false)} />
                         <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="absolute right-0 top-full mt-3 w-56" style={{ zIndex: 91, transformOrigin: 'top right' }}>
                           <div className="rounded-3xl shadow-2xl flex flex-col overflow-hidden p-3 gap-1.5" style={{ background: dark ? 'rgba(15,23,42,0.95)' : 'rgba(255,255,255,0.95)', border: '1px solid var(--border-2)', backdropFilter: 'blur(24px)' }}>
                             
                             <p className="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 mt-1" style={{ color: 'var(--text-faint)' }}>Pengaturan Obrolan</p>
                             
                             <button onClick={() => { setPersona(persona === 'sahabat' ? 'psikolog' : persona === 'psikolog' ? 'filsuf' : 'sahabat') }} className="text-left px-4 py-3.5 text-[13px] font-bold rounded-2xl flex items-center gap-3 transition-all" style={{ background: 'var(--surface-2)' }}>
                               <span className="text-lg">{persona === 'sahabat' ? '👋' : persona === 'psikolog' ? '🩺' : '🧘'}</span>
                               <span style={{ color: 'var(--text)' }}>{persona === 'sahabat' ? 'Gaya Sahabat' : persona === 'psikolog' ? 'Gaya Psikolog' : 'Gaya Filsuf'}</span>
                             </button>

                             <button onClick={() => setIsVoiceMode(!isVoiceMode)} className="text-left px-4 py-3.5 text-[13px] font-bold rounded-2xl flex items-center gap-3 transition-all" style={{ background: isVoiceMode ? 'rgba(59,130,246,0.12)' : 'var(--surface-2)' }}>
                               <span className="text-lg">{isVoiceMode ? '🔊' : '🔇'}</span>
                               <span style={{ color: isVoiceMode ? '#3b82f6' : 'var(--text)' }}>{isVoiceMode ? 'Suara Aktif' : 'Suara Mati'}</span>
                             </button>
                             
                             <button onClick={toggleTheme} className="text-left px-4 py-3.5 text-[13px] font-bold rounded-2xl flex items-center gap-3 transition-all" style={{ background: 'var(--surface-2)' }}>
                               <span className="text-lg">{dark ? '☀️' : '🌙'}</span>
                               <span style={{ color: 'var(--text)' }}>{dark ? 'Mode Terang' : 'Mode Gelap'}</span>
                             </button>

                             <div className="h-px w-full my-2" style={{ background: 'var(--border-2)' }} />
                             
                             <div className="flex gap-2 w-full mb-1">
                                <button onClick={() => { setZenMode(true); setShowMobileMenu(false) }} className="flex-1 py-3 rounded-2xl flex items-center justify-center text-xl transition-all" style={{ background: 'var(--surface-2)' }} title="Mode Fokus">🧘</button>
                                <button onClick={() => handlePlayAmbient(ambient === 'hujan' ? null : 'hujan')} className="flex-1 py-3 rounded-2xl flex items-center justify-center text-xl transition-all" style={{ background: ambient === 'hujan' ? 'rgba(59,130,246,0.15)' : 'var(--surface-2)' }}>🌧️</button>
                                <button onClick={() => handlePlayAmbient(ambient === 'api' ? null : 'api')} className="flex-1 py-3 rounded-2xl flex items-center justify-center text-xl transition-all" style={{ background: ambient === 'api' ? 'rgba(249,115,22,0.15)' : 'var(--surface-2)' }}>🔥</button>
                             </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 sm:py-8 relative" style={{ zIndex: 10 }}>
            <div className="max-w-4xl mx-auto w-full flex flex-col gap-5 sm:gap-6 relative">
                  ...
                  {idx === activeMessages.length - 1 && !loading && !pendingText && msg.suggestions && msg.suggestions.length > 0 && (
                    <div className="flex flex-wrap justify-center gap-2 my-4 px-2">
                      {msg.suggestions.map((sug, i) => (
                        <motion.button key={i} 
                          initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1, type: "spring", stiffness: 300, damping: 20 }}
                          whileHover={{ scale: 1.05, y: -2, boxShadow: '0 10px 25px rgba(59,130,246,0.2)' }} whileTap={{ scale: 0.95 }}
                          onClick={() => { setInput(sug); textareaRef.current?.focus() }}
                          className="px-5 py-2.5 rounded-full text-xs font-bold shadow-md transition-all relative overflow-hidden group"
                          style={{ 
                          ...
                          <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-blue-100 to-transparent opacity-0 group-hover:opacity-20 transform -translate-x-full group-hover:animate-[shine_1.5s_infinite]"></div>
                          {sug}
                        </motion.button>
                      ))}
                    </div>
